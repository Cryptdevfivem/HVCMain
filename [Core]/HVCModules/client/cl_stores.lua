local cfg = module("HVCModules", "configs/cfg_stores")
local inMenu = false
...
RMenu.Add("HVC:Shops", "main", RageUI.CreateMenu("Shop", "~d~Shop", GetRageUIMenuWidth(),GetRageUIMenuHeight()))
RMenu.Add("HVC:Shops", "sub", RageUI.CreateSubMenu(RMenu:Get("HVC:Shops", "main"), "Shop", "~d~Confirm Purchase", GetRageUIMenuWidth(),GetRageUIMenuHeight()))

RageUI.CreateWhile(1.0, RMenu:Get("HVC:Shops", "main"), nil, function()
    RageUI.IsVisible(RMenu:Get("HVC:Shops", "main"), true, false, true, function()
        for k, v in pairs(cfg.items) do
            if v.type == 'store' then
                ...
                end, RMenu:Get("HVC:Shops", "sub", true))
            end
        end
    ...
    RageUI.IsVisible(RMenu:Get("HVC:Shops", "sub"), true, false, true, function()
        ...
        RageUI.ButtonWithStyle("~g~Confirm Purchase",nil,  {RightLabel = "£"..getMoneyStringFormatted(currentItemPrice * amount)}, true, function(Hovered, Active, Selected)
            if Selected then
                TriggerServerEvent("HVC:BuyShopItem", currentItemID, amount)
            end
        end, RMenu:Get("HVC:Shops", "main", true))
        ...
        for k, v in pairs(cfg.shops) do
            ...
            if isInArea(v, 1.0) and inMenu == false then
                ...
                if IsControlJustPressed(0, 51) then 
                    inMenu = true
                    currentShop = k
                    RageUI.Visible(RMenu:Get("HVC:Shops", "main"), true)
                end
            end
