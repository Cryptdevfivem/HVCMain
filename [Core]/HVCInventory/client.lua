...
AddEventHandler('openBoot', function()
    ...
    if VehInRadius and IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then 
        ...
        TriggerServerEvent('HVC:FetchTrunkInventory', NVeh, NetworkGetNetworkIdFromEntity(nearestVeh))
    else
...
RegisterNetEvent("HVC:OpenHomeStorage")
AddEventHandler("HVC:OpenHomeStorage", function(toggle , houseName)
    if toggle == true then
        TriggerServerEvent('HVC:FetchPersonalInventory')
        inventoryOpen = true; 
        SetNuiFocus(true, true)
        ...
        TriggerServerEvent('HVC:FetchHouseInventory', houseName)
    else
        inventoryOpen = false;
        SetNuiFocus(false, false)
        SetNuiFocusKeepInput(false)
...
AddEventHandler('HVC:InventoryOpen', function(toggle, lootbag)
...
RegisterNUICallback('MoveBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('HVC:MoveItem', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('HVC:MoveItem', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('HVC:MoveItem', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveXBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('HVC:MoveItemX', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('HVC:MoveItemX', data.invType, data.itemId, "home") -- for housing
        end
    else 
...
RegisterNUICallback('MoveAllBtn', function(data, cb)
    if not IsLootBagOpening then
        ...
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('HVC:MoveItemAll', data.invType, data.itemId, VehTypeA, NetworkGetNetworkIdFromEntity(nearestVeh2)) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('HVC:MoveItemAll', data.invType, data.itemId, "home") -- for housing
        end
    else 
...
RegisterNetEvent('HVC:LockPick2')
AddEventHandler('HVC:LockPick2', function()
    TriggerServerEvent('HVC:LockPick')
end)

RegisterNetEvent('HVC:whatIsThis')
AddEventHandler('HVC:whatIsThis', function()
        ...
        hasDoneIt = false
               ...
               if chance == 3 then
                   ...
                   --
                       ...
                       VehTypeC = nearestVeh
                       inventoryType = 'CarBoot'
                       TriggerServerEvent('HVC:FetchTrunkInventory', GetEntityArchetypeName(nearestVeh), NetworkGetNetworkIdFromEntity(nearestVeh))                   
                   HVC.notify({"~g~You were succesful in picking this car."})
               else
                   HVC.notify({"~r~You were unsuccesful in picking this car."})
                end
