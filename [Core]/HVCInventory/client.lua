...
AddEventHandler('openBoot', function()
    ...
    if VehInRadius and IsPedInAnyVehicle(GetPlayerPed(-1), false) == false then 
        ...
        TriggerServerEvent('AQUARP:FetchTrunkInventory', NVeh, NetworkGetNetworkIdFromEntity(nearestVeh))
    else
        notify("~d~This is not your Vehicle!")
    end
end)
...
RegisterCommand('inventory', function()
    if not tHVC.isInComa({}) then
        if not inventoryOpen then
            ...
            inventoryType = nil;
            if BootCar then
                TriggerEvent('AQUARP:clCloseTrunk')
                BootCar = nil;
                VehTypeC = nil;
                VehTypeA = nil;
            end
            if IsLootBagOpening then
                ...
                TriggerServerEvent('AQUARP:CloseLootbag')
...
RegisterNetEvent("AQUARP:OpenHomeStorage")
AddEventHandler("AQUARP:OpenHomeStorage", function(toggle , houseName)
    if toggle == true then
        TriggerServerEvent('AQUARP:FetchPersonalInventory')
        inventoryOpen = true; 
        SetNuiFocus(true, true)
        ...
        TriggerServerEvent('AQUARP:FetchHouseInventory', houseName)
    else
        inventoryOpen = false;
        SetNuiFocus(false, false)
        SetNuiFocusKeepInput(false)
...
RegisterNetEvent('AQUARP:InventoryOpen')
AddEventHandler('AQUARP:InventoryOpen', function(toggle, lootbag)
...
RegisterNetEvent('AQUARP:ToggleNUIFocus')
AddEventHandler('AQUARP:ToggleNUIFocus', function(value)
    --print('focus', value)
    SetNuiFocus(value, value)
    SetNuiFocusKeepInput(value)
end)

RegisterNetEvent('AQUARP:SendSecondaryInventoryData')
AddEventHandler('AQUARP:SendSecondaryInventoryData', function(InventoryData, CurrentKG, MaxKg)
...
RegisterNetEvent('AQUARP:FetchPersonalInventory')
AddEventHandler('AQUARP:FetchPersonalInventory', function(table, CurrentKG, MaxKG)
...
RegisterNUICallback('UseBtn', function(data, cb)
    TriggerServerEvent('AQUARP:UseItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('TrashBtn', function(data, cb)
    TriggerServerEvent('AQUARP:TrashItem', data.itemId, data.invType)
    cb(true);
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)

RegisterNUICallback('GiveBtn', function(data, cb)
    TriggerServerEvent('AQUARP:GiveItem', data.itemId, data.invType)
    cb(true)
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
end)
...
RegisterNUICallback('MoveBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('AQUARP:MoveItem', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('AQUARP:MoveItem', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('AQUARP:MoveItem', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveXBtn', function(data, cb)
    if not IsLootBagOpening then
        if inventoryType == 'CarBoot' then
            TriggerServerEvent('AQUARP:MoveItemX', data.invType, data.itemId, VehTypeA) -- for vehicle
        elseif inventoryType == "Housing" then
            TriggerServerEvent('AQUARP:MoveItemX', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('AQUARP:MoveItemX', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNUICallback('MoveAllBtn', function(data, cb)
    if not IsLootBagOpening then
        ...
        elseif inventoryType == "Housing" then
            TriggerServerEvent('AQUARP:MoveItemAll', data.invType, data.itemId, "home") -- for housing
        end
    else 
        TriggerServerEvent('AQUARP:MoveItemAll', 'LootBag', data.itemId, LootBagIDNew)
    end
    PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
    cb(true)
end)
...
RegisterNetEvent('AQUARP:LockPick2')
AddEventHandler('AQUARP:LockPick2', function()
    TriggerServerEvent('AQUARP:LockPick')
end)
      ...
      local nearestVeh = HVC.getNearestVehicle({3})
        hasDoneIt = false
               ...
               notify('~y~Lockpick Progress: 10%')
                ...
                return notify('~d~You have Cancelled the Lockpick!')
               end
               Wait(6*1000)
               if not hasDoneIt then
